...
if choice == "Adding":
    ...
    moodle_id = st.text_input("Moodle ID",placeholder='Enter Moodle ID')
    ...
    if upload == "Upload":
        ...
        if uploaded_image is not None:
            ...
            if submit_btn:
                if name == "" or moodle_id == "":
                    st.error("Please enter Name and Moodle ID")
                else:
                    ret = submitNew(name, moodle_id, uploaded_image)
                    if ret == 1: 
                        st.success("Student Added")
                    elif ret == 0: 
                        st.error("Student Moodle ID already exists")
                    elif ret == -1: 
        ...
        if img_file_buffer is not None:
            ...
            if submit_btn: 
                if name == "" or moodle_id == "":
                    ...
                    ret = submitNew(name, moodle_id, cv2_img)
                    if ret == 1: 
                        st.success("Student Added")
                    elif ret == 0: 
                        st.error("Student Moodle ID already exists")
    ...
    submit_btn = st.button("Submit",key="submit_btn")
    if submit_btn:
        name, image,_ = get_info_from_id(moodle_id)
        if name == None and image == None:
            st.error("Student Moodle ID does not exist")
        else:
            st.success(f"Name of student with Moodle ID {moodle_id} is: {name}")
            st.warning("Please check the image below to make sure you are deleting the right student")
            st.image(image)
            del_btn = st.button("Delete",key="del_btn",on_click=del_btn_callback, args=(moodle_id,)) 
    ...
    submit_btn = st.button("Submit",key="submit_btn")
    if submit_btn:
        old_name, old_image, old_idx = get_info_from_id(moodle_id)
        if old_name == None and old_image == None:
            ...
            with st.form(key='my_form'):
                ...
                new_id  = col1.text_input("Moodle ID",key='new_id',value=moodle_id,placeholder='Enter new Moodle ID')
                ...
                st.form_submit_button(label='Submit',on_click=form_callback, args=(old_name, moodle_id, old_image, old_idx))